...
namespace rms
{
  ...
  RMS::RMS(size_t K, float lambda, float voxel_input, float voxel_output)
  {
    ...
    _voxel_output = voxel_output;
  }

  /*//{ sample() */
  void RMS::sample(t_points &pts)
  {
    ...
    if (pts->empty())
    {
      printf("[RMS] Empty cloud received. Nothing to sample.");
      return;
    }
  ...
  std::vector<t_gfh> RMS::computeGFH(const t_points points_in, const t_indices &indices_in)
  {
    ...
    {
      std::for_each(gfh.begin(), gfh.end(), [&max_gfh_norm](auto &gfh)
                    { gfh.second /= max_gfh_norm; });
    }
  ...
  /*//{ extractByIndices() */
  void RMS::extractByIndices(t_points &msg_inout, const t_indices &indices)
  {
    t_points msg_out(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::copyPointCloud(*msg_inout, indices, *msg_out);
    msg_inout = std::move(msg_out);
  }
  ...
  void RMS::voxelizeIndices(const float voxel_size, const t_points pc_in, t_indices &indices_inout, const bool check_NaNs)
  {
    ...
    tsl::robin_map<t_voxel, int, VoxelHash> grid;
